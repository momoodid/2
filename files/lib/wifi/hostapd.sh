...
hostapd_set_bss_options() {
	...
	config_get max_num_sta "$vif" MbssMaxStaNum 0
...
#	append "$var" "ctrl_interface=/var/run/hostapd-$phy" "$N"
	...
	case "$enc" in
		...
		*)
			...
			#combination left must be CCMP+TKIP(wap=3)
			wpa=3
			crypto="CCMP TKIP"
		;;
	esac
	...
	case "$enc" in
		...
		;;
		*psk*)
			config_get_bool sks_ppsk_enable "$vif" sks_ppsk_enable 0

			if [ "$sks_ppsk_enable" -gt 0 ]
			then
				append "$var" "sks_ppsk_enable=1" "$N"
				config_get sks_ppsk_key_file "$vif" sks_ppsk_key_file
				[ -n "$sks_ppsk_key_file" ] && append "$var" "sks_ppsk_key_file=$sks_ppsk_key_file" "$N"
				config_get sks_ppsk_table "$vif" sks_ppsk_table
				[ -n "$sks_ppsk_table" ] && append "$var" "sks_ppsk_table=$sks_ppsk_table" "$N"

				#get redis server ip/port from system config.
				sks_redis_ip=$(uci_get_state system ppsk_redis ip "")
				[ -n "$sks_redis_ip" ] && append "$var" "sks_redis_ip=$sks_redis_ip" "$N"
				sks_redis_port=$(uci_get_state system ppsk_redis port "")
				[ -n "$sks_redis_port" ] && append "$var" "sks_redis_port=$sks_redis_port" "$N"
				sks_redis_auth=$(uci_get_state system ppsk_redis auth "")
				[ -n "$sks_redis_auth" ] && append "$var" "sks_redis_auth=$sks_redis_auth" "$N"
 			else
...
#			fi
		...
		*wep*)
		...
		ccmp)
			...
			if [ -n "$suite_b" ]; then
				...
				[ -n "$sae_password" ] && append "$var" "wpa_passphrase=$sae_password" "$N"
			fi
		;;
		*)
			wpa=0
	...
	config_get macauth_enable "$vif" macauth_enable
	[ -z "$macauth_enable" ] && macauth_enable=0
	if [ $macauth_enable -ne 0 ]; then
		config_get macauth_server_addr "$vif" macauth_server_addr
		append "$var" "macauth_server_addr=$macauth_server_addr" "$N"
		config_get macauth_server_port "$vif" macauth_server_port
		append "$var" "macauth_server_port=$macauth_server_port" "$N"
		config_get macauth_server_auth_method "$vif" macauth_server_auth_method
		append "$var" "macauth_server_auth_method=$macauth_server_auth_method" "$N"
		config_get macauth_server_shared_secret "$vif" macauth_server_shared_secret
		append "$var" "macauth_server_shared_secret=$macauth_server_shared_secret" "$N"
		config_get macauth_mac_case "$vif" macauth_mac_case
		append "$var" "macauth_mac_case=$macauth_mac_case" "$N"
		config_get macauth_delimiter "$vif" macauth_delimiter
		append "$var" "macauth_delimiter=$macauth_delimiter" "$N"
		config_get avoid_auth "$vif" avoid_auth
		append "$var" "avoid_auth=$avoid_auth" "$N"
		echo [`date`] hostapd macauth: $macauth_enable >> /tmp/hcyy-hostapd
	fi
	...
	then
		...
		# hostapd doesn't support 802.11w
...
#	hostapd -P /var/run/wifi-$ifname.pid -B /var/run/hostapd-$ifname.conf
	if [ -f "/var/run/hostapd-$ifname.lock" ]; then
		 rm /var/run/hostapd-$ifname.lock
	fi
	local confname=/var/run/hostapd-$ifname.conf
	hostapd_cli -p /var/run/hostapd raw ADD bss_config=$ifname:$confname
